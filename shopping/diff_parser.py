...
def total_spent_parser(all_prices):
    price_msg = ''
    if not all_prices:
        return None
...
def do_everything(diff_result):
    ...
    ordered_msg = to_be_ordered_parser(ordered_list)
    price_msg = total_spent_parser(prices_list)

    return ["-------", change_msg, add_msg, remove_msg, ordered_msg, price_msg]
